...
Server::Server(GUIUpdater* gui)
    : gui_(gui),
      ...
      pictures_processed_(0),speed_summ_(0),send_time_summ_(0),
      receive_time_summ_(0),running_flag_(1),server_port_(0){};

void Server::SetPort(const int port){
  server_port_ = port;
}

void Server::ProcessPicture() {
  try {
  boost::asio::ip::tcp::socket sock(io_service_);
  acceptor_.accept(sock);
  boost::posix_time::ptime full_beg = boost::posix_time::microsec_clock::local_time();
  ...
  boost::posix_time::ptime receive_beg = boost::posix_time::microsec_clock::local_time();
  boost::asio::read_until(sock, buffer, file_final_sequence);
  boost::posix_time::ptime receive_end = boost::posix_time::microsec_clock::local_time();
  ...
  boost::posix_time::ptime send_beg = boost::posix_time::microsec_clock::local_time();
  while (out_file) {
  ...
  boost::asio::write(sock, boost::asio::buffer(file_final_sequence),
                     boost::asio::transfer_all(), ignored_error);
  boost::posix_time::ptime send_end = boost::posix_time::microsec_clock::local_time();
  ...
  boost::posix_time::ptime full_end  = boost::posix_time::microsec_clock::local_time();
  boost::posix_time::time_duration full_time = full_end - full_beg;
  CalcAverageSpeed(full_time.total_milliseconds(), time_receive.total_milliseconds(),
...
void Server::Run() {
  boost::asio::ip::tcp::endpoint endpoint(boost::asio::ip::tcp::v4(), server_port_);
  acceptor_.open(endpoint.protocol());
  acceptor_.set_option(boost::asio::ip::tcp::acceptor::reuse_address(true));
  acceptor_.bind(endpoint);
  acceptor_.listen();
  while ( running_flag_ ) {
    clients_.emplace_back(boost::thread(&Server::ProcessPicture , this));
  };
};
